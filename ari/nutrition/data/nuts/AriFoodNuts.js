...
//
// Purpose:
//   Collection controller for ARI Nutrition's nuts pathway.
//
// Pathway:
//   ari/nutrition/data/nuts/
//
// Scope:
//   Includes culinary nuts and common nut products such as:
//   - Almonds
//   - Cashews
//   - Walnuts
//   - Pecans
...
//
// Taxonomy note:
//   ARI uses a culinary/nutrition definition rather than a
//   strict botanical definition. Peanuts are included here
//   because users commonly treat them as nuts.
//
// Architecture:
//   GENERIC FIRST.
//
//   AriFoodNutsCore is the required pathway dataset.
//   AriFoodNutBrands is optional and may later provide
//   packaged/branded nut products.
//
// Responsibilities:
//   - Track required and optional Nuts child modules.
//   - Record module load/failure state.
//   - Aggregate registration counts.
//   - Expose readiness, coverage, health, and diagnostics.
//   - Keep nut data isolated from grains, oils, condiments,
//     snacks, and branded packaged foods.
//
// Non-responsibilities:
//   - Does not contain individual nut records.
//   - Does not infer roasting oil, salt, coatings, or flavor.
//   - Does not merge nut butters into plain nut records.
...
//   - Common household serving: 1 oz / 28 g
//   - Keep raw and dry-roasted records separate.
//   - Salted, honey-roasted, candied, chocolate-coated,
//     flavored, and oil-roasted products should remain
//     separate branded or prepared-product records.
//
// Dependencies:
//   - AriFoodRegistry v2+
// =====================================================

(function initializeAriFoodNuts(global) {
  "use strict";

  const VERSION = "1.0.0";
  const COLLECTION_ID = "nuts";
  const COLLECTION_NAME = "AriFoodNuts";
  ...
  function healthCheck() {
    ...
    if (
      ...
      issues.push({
        ...
        severity: "warning",
        message:
          "Required Nuts modules report ready but registration total is zero."
      });
    }
  ...
  global.AriFoodNuts = Object.freeze({
    VERSION,
    COLLECTION_ID,
    COLLECTION_NAME,
  ...
  emitEvent(
    "ari:food-nuts-initialized",
    {
      ...
      architecture: {
        strategy: "generic-first",
        brandedModuleRequired: false
      }
    }
  );

  console.info(
    `[ARI Nutrition] ${COLLECTION_NAME} v${VERSION} initialized with ${REQUIRED_MODULES.length} required module(s) and ${OPTIONAL_MODULES.length} optional module(s).`
  );
