...
//
// Purpose:
//   Collection controller for ARI Nutrition's solid and
//   semi-solid culinary fat pathway.
//
// Pathway:
//   ari/nutrition/data/fats/
//
// Scope:
//   Includes:
//   - Butter
...
//   HYBRID.
//
//   AriFoodFatsCore is the required generic fallback
//   dataset.
//
//   AriFoodFatBrands is also required because butter,
//   margarine, plant-based spreads, and shortening can
//   vary substantially by product, serving size, sodium,
//   saturated fat, and formulation.
//
// Responsibilities:
//   - Track required Fats child modules.
//   - Record module load/failure state.
//   - Aggregate registration counts.
//   - Expose readiness, coverage, health, and diagnostics.
//   - Keep solid/semi-solid fats isolated from oils,
//     condiments, dairy, and nut products.
//
// Non-responsibilities:
//   - Does not contain individual fat records.
//   - Does not infer cooking absorption.
//   - Does not estimate how much fat remained on food.
...
(function initializeAriFoodFats(global) {
  "use strict";

  const VERSION = "1.0.0";
  const COLLECTION_ID = "fats";
  const COLLECTION_NAME = "AriFoodFats";
  ...
  function healthCheck() {
    ...
    if (
      ...
      issues.push({
        ...
        severity: "warning",
        message:
          "Required Fats modules report ready but registration total is zero."
      });
    }
  ...
  function getDiagnostics() {
    return {
      ...
      taxonomy: {
        butterIncluded: true,
        gheeIncluded: true,
        margarineIncluded: true,
        plantBasedSpreadsIncluded: true,
        shorteningIncluded: true,

        cookingOilsIncluded: false,
        mayonnaiseIncluded: false,
        nutButtersIncluded: false,
        cheeseIncluded: false,
        saladDressingsIncluded: false
      },
