// =====================================================
// ARI REBIRTH
// File: AriFoodDairy.js
// Version: 1.0.0
//
// Purpose:
//   Collection controller for ARI Nutrition dairy
//   submodules.
//
// Architecture:
//   AriFoodRegistry
...
//
// Strategy:
//   Dairy is intentionally brand-first.
//
//   - AriFoodDairyCore contains a small set of generic
//     fallback dairy foods.
//   - Brand modules contain the products users are most
//     likely to purchase and log from grocery stores.
//   - Branded products should preserve exact manufacturer
//     label servings and normalize nutrition to 100 g.
//
// Responsibilities:
//   - Define the expected dairy-data modules.
...
//   - Does not contain dairy food records.
...
//   2. AriFoodDairy.js
//   3. AriFoodDairyCore.js
//   4. AriFoodMilkBrands.js
//   5. AriFoodYogurtBrands.js
//   6. AriFoodCheeseBrands.js
//   7. AriFoodDairyBrands2.js
//   8. AriFoodDairyBrands3.js
// =====================================================

(function initializeAriFoodDairy(global) {
  "use strict";

  const VERSION = "1.0.0";
  const COLLECTION_ID = "dairy";
  const COLLECTION_NAME = "AriFoodDairy";
  ...
  function healthCheck() {
    ...
    if (failedModules.length > 0) {
      issues.push(
        `${failedModules.length} dairy module(s) failed.`
      );
    }
    ...
    if (
      ...
      issues.push(
        "The dairy collection completed with one or more failures."
      );
    }
    ...
    if (
      ...
      issues.push(
        "All dairy modules report loaded but no dairy records are visible in AriFoodRegistry."
      );
    }
    ...
    if (
      ...
      if (emptyLoadedModules.length > 0) {
        issues.push(
          `${emptyLoadedModules.length} loaded dairy module(s) have zero visible registry records.`
        );
      }
    }
  ...
  console.info(
    `[ARI Nutrition] ${COLLECTION_NAME} v${VERSION} initialized with ${EXPECTED_MODULES.length} expected dairy modules.`
  );
