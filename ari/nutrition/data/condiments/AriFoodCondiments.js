// =====================================================
// ARI REBIRTH
// File: AriFoodCondiments.js
// Version: 1.0.0
//
// Purpose:
//   Collection controller for ARI Nutrition's condiments,
//   sauces, dressings, and table-sauce pathway.
//
// Pathway:
//   ari/nutrition/data/condiments/
//
// Scope:
//   Includes common condiments and pourable sauces such as:
//   - Ketchup
//   - Mustard
//   - Mayonnaise
//   - Barbecue sauce
...
//   HYBRID.
//
//   AriFoodCondimentsCore is the required generic fallback
//   dataset.
//
//   AriFoodCondimentBrands is also expected because branded
//   condiment nutrition can vary substantially in calories,
//   sugar, sodium, fat, and serving size.
//
// Responsibilities:
//   - Track required and optional Condiments child modules.
//   - Record module load/failure state.
//   - Aggregate registration counts.
//   - Expose readiness, coverage, health, and diagnostics.
//   - Keep condiments isolated from oils, dairy, spreads,
//     beverages, and prepared meals.
//
// Non-responsibilities:
//   - Does not contain individual condiment records.
//   - Does not infer serving amounts used on meals.
//   - Does not merge plain cooking oils into condiments.
//   - Does not merge standalone cheeses, butter, or cream.
...
//   - Preserve common household measures such as:
//     1 tsp, 1 tbsp, 2 tbsp, packet, cup, or fluid ounce.
//   - Preserve manufacturer serving size for branded items.
//   - Normalize all records to 100 g for registry consistency.
//
// Dependencies:
//   - AriFoodRegistry v2+
// =====================================================

(function initializeAriFoodCondiments(global) {
  "use strict";

  const VERSION = "1.0.0";
  const COLLECTION_ID = "condiments";
  const COLLECTION_NAME = "AriFoodCondiments";

  const REQUIRED_MODULES = Object.freeze([
    "AriFoodCondimentsCore",
    "AriFoodCondimentBrands"
  ]);
  ...
  function markModuleLoaded(
    ...
    emitEvent(
      "ari:food-condiments-module-update",
      {
        action: "loaded",
        module: clone(next),
        totals: getTotals(),
  ...
  function healthCheck() {
    ...
    for (const state of moduleState.values()) {
      if (
        ...
        issues.push({
          ...
          message:
            state.error ||
            "Required Condiments module failed."
        });
      } else if (
        state.status === "failed" &&
        !state.required
        ...
        issues.push({
          ...
          message:
            state.error ||
            "Optional Condiments module failed."
        });
      }
    }
    ...
    if (
      ...
      issues.push({
        ...
        severity: "warning",
        message:
          "Required Condiments modules report ready but registration total is zero."
      });
    }
  ...
  function getDiagnostics() {
    return {
      ...
      taxonomy: {
        ketchupIncluded: true,
        mustardIncluded: true,
        mayonnaiseIncluded: true,
        barbecueSauceIncluded: true,
        hotSauceIncluded: true,
        soySauceIncluded: true,
        dressingsIncluded: true,
        salsaIncluded: true,
        relishIncluded: true,
        cookingOilsIncluded: false,
        butterIncluded: false
      },
  ...
  emitEvent(
    "ari:food-condiments-initialized",
    {
      version: VERSION,
      collectionId: COLLECTION_ID,
      collectionName:
