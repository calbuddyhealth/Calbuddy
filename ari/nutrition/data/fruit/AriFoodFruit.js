// =====================================================
// ARI REBIRTH
// File: AriFoodFruit.js
// Version: 1.0.0
//
// Purpose:
//   Collection controller for ARI Nutrition fruit
//   submodules.
//
// Architecture:
//   AriFoodRegistry
...
//   ├── AriFoodBerries
//   ├── AriFoodCitrus
//   ├── AriFoodTropicalFruit
//   ├── AriFoodStoneFruit
//   ├── AriFoodMelons
//   └── AriFoodDriedFruit
//
// Strategy:
//   Fruit is generic-first.
//
//   - Whole and minimally processed fruit records are
//     primarily sourced from USDA references.
//   - Canonical nutrition basis is 100 g edible portion.
//   - Real-world household servings may be included when
//     reliable gram equivalents are available.
//   - Branded fruit cups, sweetened dried fruit, canned
//     fruit in syrup, smoothie blends, and other packaged
//     products should live in separate branded/processed
//     modules later.
//
// Responsibilities:
//   - Define the expected fruit-data modules.
...
//   - Does not contain fruit food records.
...
//   2. AriFoodFruit.js
//   3. AriFoodCommonFruit.js
//   4. AriFoodBerries.js
//   5. AriFoodCitrus.js
//   6. AriFoodTropicalFruit.js
//   7. AriFoodStoneFruit.js
//   8. AriFoodMelons.js
//   9. AriFoodDriedFruit.js
// =====================================================

(function initializeAriFoodFruit(global) {
  "use strict";

  const VERSION = "1.0.0";
  const COLLECTION_ID = "fruit";
  const COLLECTION_NAME = "AriFoodFruit";
  ...
  function healthCheck() {
    ...
    if (failedModules.length > 0) {
      issues.push(
        `${failedModules.length} fruit module(s) failed.`
      );
    }
    ...
    if (
      ...
      issues.push(
        "The fruit collection completed with one or more failures."
      );
    }
    ...
    if (
      ...
      issues.push(
        "All fruit modules report loaded but no fruit records are visible in AriFoodRegistry."
      );
    }
    ...
    if (
      ...
      if (emptyLoadedModules.length > 0) {
        issues.push(
          `${emptyLoadedModules.length} loaded fruit module(s) have zero visible registry records.`
        );
      }
    }
  ...
  console.info(
    `[ARI Nutrition] ${COLLECTION_NAME} v${VERSION} initialized with ${EXPECTED_MODULES.length} expected fruit modules.`
  );
