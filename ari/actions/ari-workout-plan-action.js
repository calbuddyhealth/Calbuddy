...
(() => {
  ...
  function resolveRequestedDate(text = "", now = new Date()) {
    ...
    if (/\btomorrow\b/.test(value)) {
      const date = new Date(base); date.setDate(date.getDate() + 1); return formatDateKey(date);
    }
    ...
    for (let target = 0; target < weekdays.length; target += 1) {
      ...
      const date = new Date(base); date.setDate(date.getDate() + delta); return formatDateKey(date);
    }
    return null;
  }
  ...
  function isTrainingDecision(decision = {}) {
    ...
    return clean(decision.domain) === "training" && ["plan_workout", "edit_workout"].includes(action) && decision.needs_clarification !== true && Number(decision.confidence || 0) >= 0.8;
  }

  function focusDefinition(focusText = "") {
    const value = clean(focusText).toLowerCase();
    const rules = [
    ...
    return rules.find(rule => rule.test.test(value)) || { id: "custom", title: value ? `${value.charAt(0).toUpperCase()}${value.slice(1)} Workout` : "Workout", goal: "general_fitness", bodyParts: [], modules: [] };
  }

  function extractWorkoutRequest(message = "", decision = {}) {
    const entities = decision?.entities || {};
    const focus = focusDefinition(entities.workout_focus);
    const durationValue = Number(entities.duration_minutes);
    const durationMinutes = Number.isFinite(durationValue) && durationValue >= 10 ? Math.min(180, Math.round(durationValue)) : 45;
    const difficultyText = clean(entities.difficulty).toLowerCase();
    const difficulty = ["beginner", "intermediate", "advanced"].includes(difficultyText) ? difficultyText : "intermediate";
    return {
      focusId: focus.id,
  ...
  function getPendingWorkout() {
    try {
      const pending = CalBuddy.getPendingAction?.();
      return pending?.action_type === "plan_workout" && pending?.status !== "cancelled" ? pending : null;
    } catch { return null; }
  }

  function isPendingWorkoutRevision(message = "", decision = {}, pending = null) {
    if (!pending || !isTrainingDecision(decision)) return false;
    const action = clean(decision.action);
    if (action === "edit_workout") return true;
    if (action !== "plan_workout") return false;
    const text = clean(message).toLowerCase();
    const entities = decision?.entities || {};
    const hasWorkoutEntity = Boolean(clean(entities.workout_focus) || clean(entities.workout_date_text) || entities.duration_minutes != null || clean(entities.difficulty) || clean(entities.exercise));
    const replacementCue = /\b(actually|instead|change|make it|make that|switch|swap|replace|shorter|longer|easier|harder|minutes?|mins?|tomorrow|today)\b/.test(text);
    return hasWorkoutEntity || replacementCue;
  }

  function mergePendingWorkoutRequest(message = "", decision = {}, pending = {}) {
    const priorPayload = pending?.payload || {};
    const priorOptions = priorPayload?.builder_options || {};
    const entities = { ...(decision?.entities || {}) };
    const explicitFocus = clean(entities.workout_focus);
    const explicitDuration = entities.duration_minutes != null && Number.isFinite(Number(entities.duration_minutes));
    const explicitDifficulty = clean(entities.difficulty);
    const priorFocus = clean(priorPayload.focus_id || priorOptions.focusId);
    if (!explicitFocus && priorFocus) entities.workout_focus = priorFocus;
    if (!explicitDuration && Number.isFinite(Number(priorOptions.durationMinutes))) entities.duration_minutes = Number(priorOptions.durationMinutes);
    if (!explicitDifficulty && clean(priorOptions.difficulty)) entities.difficulty = clean(priorOptions.difficulty);

    const mergedDecision = { ...decision, domain: "training", action: "plan_workout", entities };
    const rebuilt = extractWorkoutRequest(message, mergedDecision);
    const builderOptions = { ...priorOptions, ...rebuilt.builderOptions };
    if (!explicitFocus) {
      builderOptions.title = priorOptions.title || rebuilt.builderOptions.title;
      builderOptions.goal = priorOptions.goal || rebuilt.builderOptions.goal;
      builderOptions.bodyParts = Array.isArray(priorOptions.bodyParts) ? priorOptions.bodyParts : rebuilt.builderOptions.bodyParts;
      builderOptions.modules = Array.isArray(priorOptions.modules) ? priorOptions.modules : rebuilt.builderOptions.modules;
      builderOptions.focusId = priorFocus || rebuilt.focusId;
    }
    if (!explicitDuration) builderOptions.durationMinutes = Number(priorOptions.durationMinutes || rebuilt.builderOptions.durationMinutes);
    builderOptions.includeWarmup = builderOptions.durationMinutes >= 15;
    builderOptions.includeCooldown = builderOptions.durationMinutes >= 20;
    builderOptions.requestedFromMessage = clean(message);
    if (clean(entities.exercise)) builderOptions.requestedExercise = clean(entities.exercise);

    const scheduledDate = resolveRequestedDate(clean(entities.workout_date_text) || message) || clean(priorPayload.scheduled_date);
    const focusId = clean(builderOptions.focusId || rebuilt.focusId || priorPayload.focus_id) || "custom";
    return { scheduledDate, focusId, builderOptions, decision: mergedDecision };
  }

  function buildPendingActionFromOptions(message, decision, scheduledDate, mode, focusId, builderOptions) {
    ...
    const title = clean(builderOptions?.title) || focusDefinition(focusId).title;
    const duration = Number(builderOptions?.durationMinutes);
    const durationLabel = Number.isFinite(duration) ? `${duration}-minute ` : "";
    return {
      ...
      payload: {
        ...
        intent_router: { domain: decision.domain, intent: decision.intent, target: decision.target, action: decision.action, confidence: decision.confidence, router_version: decision.router_version || null }
      },
      confirmation_text: `${verb} ${durationLabel}${title}${suffix}`
    };
  }
  ...
  function buildPendingAction(message, decision, scheduledDate, mode = "create") {
    ...
    return buildPendingActionFromOptions(message, decision, scheduledDate, mode, workout.focusId, workout.builderOptions);
  }
  ...
  async function getController() {
    ...
    if (!controller?.init || !controller?.getDate || !controller?.buildWorkoutForDate || !controller?.setBuiltWorkoutForDate) throw new Error("Training controller is unavailable.");
    await controller.init();
    return controller;
  }

  function hasWorkout(day) { return Boolean(day?.type === "workout" && Array.isArray(day?.exercises) && day.exercises.length > 0); }
  function describeWorkout(day) { return clean(day?.title) || clean(day?.focusLabel) || clean(day?.focusId) || "a workout"; }

  async function inspectDate(scheduledDate) {
    const controller = await getController();
    const day = controller.getDate(scheduledDate);
    return hasWorkout(day) ? { day, title: describeWorkout(day), scheduled_date: scheduledDate, dateLabel: formatDateLabel(scheduledDate) } : null;
  }
  ...
  CalBuddy.planWorkoutFromAri = async function planWorkoutFromAri(payload = {}) {
    ...
    if (!parseDateKey(scheduledDate)) return { success: false, reply: "I need an exact workout date before I can change that plan." };
    const controller = await getController();
    ...
    if (existing && mode === "create") return { success: false, conflict: true, reply: `${describeWorkout(existingDay)} is already planned for ${formatDateLabel(scheduledDate)}. I didn’t change it.` };
    const builderOptions = payload.builder_options && typeof payload.builder_options === "object" ? { ...payload.builder_options } : {};
    ...
    if (existing && mode === "add") {
      workoutToSave = { ...built, title: `${describeWorkout(existingDay)} + ${clean(built.title) || "Added Training"}`, exercises: [...(Array.isArray(existingDay.exercises) ? existingDay.exercises : []), ...(Array.isArray(built.exercises) ? built.exercises : [])] };
    }
    const saved = controller.setBuiltWorkoutForDate(scheduledDate, workoutToSave, { focusId: mode === "add" ? clean(existingDay?.focusId) || clean(payload.focus_id) || "custom" : clean(payload.focus_id) || "custom" });
  ...
  function install() {
    ...
    CalBuddy.executeAction = async function ariWorkoutExecutor(action = {}) {
      ...
      if (type === "plan_workout") {
        if (action.source && action.source !== SOURCE) throw new Error("Rejected non-canonical workout action source.");
    ...
    CalBuddy._askAriInternal = async function ariWorkoutActionRouter(input = {}) {
      ...
      if (pendingWorkout && isPendingWorkoutRevision(message, decision, pendingWorkout)) {
        ...
        return {
          ...
          workoutProposalSuperseded: true,
          supersededActionId: pendingWorkout.id || null,
      ...
      if (storedConflict && choice) {
        if (choice === "edit") {
          localStorage.setItem(EDIT_KEY, JSON.stringify({ scheduled_date: storedConflict.scheduled_date, existing_title: storedConflict.existing_title, saved_at: new Date().toISOString() }));
          clearConflict();
          return { reply: `I’ll keep ${storedConflict.existing_title} intact. Tell me exactly what you want changed.`, pendingAction: null, emotion: "coach", workoutEditMode: true, scheduled_date: storedConflict.scheduled_date, intentDecision: decision };
        }
      ...
      if (!isTrainingDecision(decision)) return await previousAskInternal(input);
      ...
      if (!requestedDate) return { reply: "What day do you want me to put that workout on — today, tomorrow, or a specific day?", pendingAction: null, emotion: "coach", workoutDateRequired: true, intentDecision: decision };
      ...
      if (existing) {
        saveConflict({ scheduled_date: requestedDate, requested_message: message, existing_title: existing.title, intent_decision: decision });
    ...
    Object.defineProperty(CalBuddy, INSTALL_FLAG, { configurable: false, enumerable: false, value: true });
