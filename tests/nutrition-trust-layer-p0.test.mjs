...
test("Nutrition trust layer is valid JavaScript and loads only on Nutrition", () => {
  ...
  assert.match(auth, /js\/nutrition-trust-layer\.js\?v=1\.0\.0/);
...
test("suspicious nutrition is flagged but users retain final control", () => {
  assert.match(trust, /function detectAnomalies/);
  assert.match(trust, /calories < 100/);
  assert.match(trust, /macroCalories = protein \* 4 \+ carbs \* 4 \+ fat \* 9/);
...
test("legacy partially-eaten planned rows are repaired without altering consumed meal history", () => {
