...
test("Home cache chain points at current quota-aware runtime and bridge assets", () => {
  assert.match(homeSource, /js\/auth\.js\?v=1\.10\.18/);
  assert.match(homeSource, /js\/home-resilience\.js\?v=1\.3\.6/);
  assert.match(authSource, /account-isolation-guard\.js\?v=1\.0\.0/);
  assert.match(authSource, /ari-central-intent-router\.js\?v=1\.5\.5/);
  assert.match(routerSource, /ari\/runtime\/ari-runtime-controller\.js\?v=1\.3\.10/);
...
test("Home loader is version-aware, dual-namespace aware, and bounded", () => {
  ...
  assert.match(resilienceSource, /REQUIRED_RUNTIME_VERSION\s*=\s*"1\.3\.10"/);
