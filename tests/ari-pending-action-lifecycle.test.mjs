...
test("successful vNext confirmations clear the legacy pending-action mirror", () => {
  assert.match(runtime, /const VERSION = "1\.3\.12"/);
...
test("Home requires and cache-busts the quota-aware runtime path", () => {
  assert.match(resilience, /Version: 1\.3\.8/);
  assert.match(resilience, /REQUIRED_RUNTIME_VERSION = "1\.3\.12"/);
  assert.match(home, /js\/home-resilience\.js\?v=1\.3\.8/);
});