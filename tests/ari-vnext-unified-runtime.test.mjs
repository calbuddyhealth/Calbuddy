...
assert.match(runtime, /ari-vnext-operation-registry\.js\?v=2\.0\.1/, "runtime must boot the canonical OperationRegistry");
...
assert.match(runtime, /ari-vnext-reference-state\.js\?v=1\.3\.0/, "runtime must boot the universal reference lifecycle");
assert.match(runtime, /AriVNextOperationRegistry\?\.ready === true/, "runtime must wait for canonical registry readiness");
...
assert.doesNotMatch(runtime, /ari-vnext-action-adapter\.js/, "retired general ActionAdapter must not return to runtime boot");
...
assert.doesNotMatch(mealAdapter, /prepareCalBuddyAction\s*=/, "Meal Plan must not monkey-patch a legacy general adapter");
