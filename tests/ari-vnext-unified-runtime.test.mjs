...
assert.match(router, /ari-runtime-controller\.js\?v=1\.3\.12/, "shared Home/Nutrition router must boot the quota-aware unified runtime controller");
...
assert.match(auth, /ari-central-intent-router\.js\?v=1\.5\.6/, "auth bootstrap must request the quota-aware shared router version");
