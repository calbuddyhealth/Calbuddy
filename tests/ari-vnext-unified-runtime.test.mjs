...
assert.match(router, /ari-runtime-controller\.js\?v=1\.3\.2/, "shared Home/Nutrition router must boot the hardened unified runtime controller");
...
assert.match(auth, /ari-central-intent-router\.js\?v=1\.5\.1/, "auth bootstrap must request the hardened unified shared router version");
