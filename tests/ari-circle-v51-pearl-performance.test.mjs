...
test("current Circle V5 presentation cascade ends in Minimal Premium", () => {
  ...
  assert.match(minimal, /ARI CIRCLE V5\.2\.4 — MINIMAL PREMIUM/);
...
test("Feed loads the current V5 cascade without Profile compatibility presentation", () => {
  ...
  const minimalIndex = feed.indexOf("ari-circle-v5-minimal-premium.css?v=5.2.4");
  assert.ok(realWorldIndex >= 0 && authorityIndex > realWorldIndex, "visual authority must load after the V5 base");
  assert.ok(minimalIndex > authorityIndex, "Minimal Premium must load last");
