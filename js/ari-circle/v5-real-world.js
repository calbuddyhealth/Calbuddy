...
(() => {
  ...
  function isCirclePath() {
    ...
    return path.includes("ari-circle");
  }
  ...
  function normalizeDrawer() {
    document.querySelectorAll(".circle-v5-menu__item").forEach((item) => {
      ...
      if (text === "Find People" || text === "Buddies" || text === "Partners") {
    ...
    document.querySelectorAll(".circle-v5-menu__items").forEach((items) => {
      if (items.querySelector('[data-circle-v5-profile-link="true"]')) return;
  ...
  function watch() {
    ...
    observer = new MutationObserver((mutations) => {
      if (mutations.some((mutation) => mutation.addedNodes.length || mutation.type === "attributes")) queueRun();
    });
