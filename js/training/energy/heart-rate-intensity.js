...
const MAX_HR_FORMULA = Object.freeze({
  /*
   * Simple legacy estimate.
   * Kept as the default because it is widely understood
   * and requires only age.
   */
  SIMPLE:
    "220_minus_age",

  /*
   * Optional alternate formula for future use.
   */
  TANAKA:
    "tanaka"
});
...
function classifyPercent(
   ...
   * ARI intensity bands.
   *
   * These are intentionally broader than laboratory zones.
   * They are used for workout categorization and calorie
   * estimation, not diagnosis or clinical decision-making.
   */
  if (
    resolved < 0.40
  ) {
...
function toCalorieIntensity(
  intensityId
) {
  /*
   * calorie-calculator.js currently works primarily with
   * light / moderate / vigorous strength buckets.
   *
   * Collapse the more granular HR zones into those buckets
   * for calorie estimation while preserving the original
   * intensityId in UI/state.
   */
  switch (
    intensityId
  ) {
...
const AriTrainingHeartRateIntensity =
  Object.freeze({
    version:
      VERSION,
