...
function isLocalSession() {
  return Boolean(
    state.activeSession &&
    String(state.activeSession.id).startsWith("local_")
  );
}
...
async function refresh() {
  const previousToday = state.todayDateKey;
  ...
  if (
    ...
    state.selectedDateKey = newToday;
    persistSelectedDate();
  }
  ...
  await restoreOpenSession({ preserveCurrent: true });
...
function createMonthCalendarDay(date, visibleMonth) {
  const dateKey = getLocalDateKey(date);
  const template = elements.trainingCalendarDayTemplate;
  let button;
  ...
  if (template?.content) {
    ...
    button = document.createElement("button");
    button.type = "button";
    button.className = "ari-training-calendar-day";
...
async function renderSelectedDay() {
  const token = ++state.selectedDayRenderToken;
...
function hidePrimaryDayStates() {
  ...
  setHidden(elements.todaysTrainingRestDay, true);
  setHidden(elements.todaysTrainingCompletedDay, true);
  ...
  try {
    ...
    for (const previous of previousExercises || []) {
      await createSessionExercise({
        ...
        source: "ad_hoc",
        sortOrder: previous.sort_order,
        completionMode: previous.completion_mode,
...
function renderCurrentExercise() {
  ...
  setText(
    ...
    `${Math.max(1, position + 1)} / ${activeExercises.length || 1}`
  );
...
function renderSessionExerciseQueue() {
  ...
  if (!container || !state.activeSession) {
    return;
  }
...
function createSessionExerciseQueueRow(exercise) {
  ...
  if (template?.content) {
    ...
    setTextWithin(
      ...
      getExerciseStateLabel(exercise.status)
    );
  } else {
    button = document.createElement("button");
    button.type = "button";
  ...
  button.dataset.exerciseId = exercise.id;
  button.dataset.status = exercise.status;
  button.dataset.action = "select-session-exercise";
...
function openExercisePicker() {
  ...
  window.setTimeout(() => {
    elements.sessionExerciseSearchInput?.focus();
  }, 30);
}
...
function createExerciseSearchResult(exercise) {
  ...
  button.dataset.exerciseId = exercise.id;
  button.dataset.action = "add-session-exercise";
...
async function syncWeeklyProgressFromSessionExercise(exercise) {
  ...
  for (const set of exercise.sets || []) {
    WorkoutProgressStore.setSetCompleted({
      day: weekday,
      exerciseId: exercise.exercise_id,
...
function groupSessionsByDate(records) {
  ...
  for (const record of records) {
    if (!map.has(record.local_date)) {
      map.set(record.local_date, []);
    }

    map.get(record.local_date).push(record);
  }
...
function createHistoryWorkoutElement(record) {
  ...
  setTextWithin(
    ...
    `${formatNumber(record.estimated_calories || 0)} kcal`
  );
...
function renderTrainingProfile() {
  ...
  setText(
    ...
    state.profileMaxHeartRateSource === "confirmed"
      ? "Confirmed max"
      : state.profileMaxHeartRateSource === "estimated"
        ? "Estimated from age"
        : "No max HR available"
  );
}
...
function handleTrainingMenuClick(event) {
...
function clearRuntimeTimers() {
  ...
  if (state.dateWatcherId) {
    clearInterval(state.dateWatcherId);
    state.dateWatcherId = null;
  }
}
...
function handleDateRollover() {
  ...
  if (newToday === state.todayDateKey) {
    return;
  }
  ...
  if (
    !state.activeSession &&
    state.selectedDateKey === oldToday
  ) {
    state.selectedDateKey = newToday;
    persistSelectedDate();
  }
...
function restoreLocalSessionCache() {
  ...
  if (
    !parsed?.session ||
    parsed.session.status === "completed" ||
    parsed.session.status === "abandoned"
  ) {
    return null;
  }
...
function getPlanReference() {
  return (
    state.plan?.planId ||
    state.plan?.metadata?.sourceTemplateId ||
    "local-plan"
  );
}
