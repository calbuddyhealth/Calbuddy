...
(() => {
  ...
  const FOOD_DATA_SCRIPTS = Object.freeze([
    ...
    "ari/nutrition/data/branded/AriFoodTopBrandsBatch5.js?v=1.0.0",
    ...
    "ari/nutrition/data/prepared-meals/AriFoodEverydaySoupsMeals2.js?v=1.0.0",
  ...
  const state = { status: "idle", startedAt: 0, readyAt: 0, promise: null, error: null, loadedModules: 0 };
  function setFoodStatus(text, stateName = "loading") { const container = document.getElementById("manualFoodSystemStatus"); const label = document.getElementById("manualFoodSystemStatusText"); if (label) label.textContent = String(text || ""); if (container) container.dataset.state = stateName; }
  function existingScript(src) { const clean = String(src || "").split("?")[0]; return Array.from(document.scripts).find((script) => { const value = script.getAttribute("src") || ""; return value === src || value.split("?")[0] === clean; }); }
  function loadScript(src, { ordered = true } = {}) { const existing = existingScript(src); if (existing) return Promise.resolve(existing); return new Promise((resolve, reject) => { const script = document.createElement("script"); script.src = src; script.async = !ordered; script.dataset.ariNutritionLazy = "true"; script.addEventListener("load", () => resolve(script), { once: true }); script.addEventListener("error", () => reject(new Error(`Could not load ${src}`)), { once: true }); document.head.appendChild(script); }); }
  function yieldToBrowser() { return new Promise((resolve) => { const finish = () => window.setTimeout(resolve, 0); if (document.visibilityState === "visible" && typeof window.requestAnimationFrame === "function") { window.requestAnimationFrame(finish); return; } window.setTimeout(resolve, 16); }); }
  async function loadFoodDataInBatches() { for (let index = 0; index < FOOD_DATA_SCRIPTS.length; index += FOOD_BATCH_SIZE) { const batch = FOOD_DATA_SCRIPTS.slice(index, index + FOOD_BATCH_SIZE); await Promise.all(batch.map((src) => loadScript(src))); state.loadedModules += batch.length; setFoodStatus(`FOOD SEARCH LOADING ${Math.min(state.loadedModules, FOOD_DATA_SCRIPTS.length)}/${FOOD_DATA_SCRIPTS.length}`, "loading"); await yieldToBrowser(); } }
  function announceReady() { try { window.initializeNutritionFoodSystem?.(); } catch (error) { console.warn("[ARI Nutrition Food Loader] food status refresh failed", error); } const nameInput = document.getElementById("mealName"); if (nameInput?.value?.trim()) { try { window.scheduleManualFoodSearch?.(); } catch {} } try { window.dispatchEvent(new CustomEvent("ari:nutritionFoodReady", { detail: { version: VERSION, foodCount: window.AriFoodRegistry?.count?.() || 0, loadMs: Math.max(0, Math.round(state.readyAt - state.startedAt)) } })); } catch {} }
  async function start() { if (state.status === "ready") return true; if (state.promise) return state.promise; state.status = "loading"; state.startedAt = performance.now(); state.error = null; state.loadedModules = 0; setFoodStatus("FOOD SEARCH LOADING", "loading"); state.promise = (async () => { try { await loadScript(REGISTRY_SCRIPT); await yieldToBrowser(); await loadFoodDataInBatches(); for (const src of ENGINE_SCRIPTS) { await loadScript(src); await yieldToBrowser(); } state.readyAt = performance.now(); state.status = "ready"; announceReady(); return true; } catch (error) { state.status = "error"; state.error = error; state.promise = null; console.error("[ARI Nutrition Food Loader]", error); setFoodStatus("MANUAL MODE", "offline"); return false; } })(); return state.promise; }
  function markIdle() { if (state.status === "idle") setFoodStatus("TAP FOOD NAME TO SEARCH", "idle"); }
  document.addEventListener("pointerdown", (event) => { if (event.target?.closest?.("#mealFoodSearchShell")) start().catch(() => {}); }, { capture: true, passive: true });
  document.addEventListener("focusin", (event) => { if (event.target?.id === "mealName") start().catch(() => {}); }, { capture: true });
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", markIdle, { once: true }); else markIdle();
  window.AriNutritionFoodLoader = Object.freeze({ version: VERSION, start, getStatus: () => ({ status: state.status, error: state.error?.message || null, foodCount: window.AriFoodRegistry?.count?.() || 0, loadedModules: state.loadedModules, totalModules: FOOD_DATA_SCRIPTS.length }) });
})();
