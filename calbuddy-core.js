...
<title>ARI | CalBuddy Health</title>
<link rel="stylesheet" href="style.css" />
</head>

<body>
<main class="app ari-v1-home" id="ariApp">

<header class="top-bar ari-header">
  <button class="menu-btn" type="button" onclick="toggleMenu()">☰</button>

  <div class="ari-brand-center">
    <div class="ari-header-avatar-shell" id="ariHeaderAvatarShell">
      <img id="ariHeaderAvatar" src="assets/ari/avatar-idle-open.png" alt="Ari avatar" />
    </div>

    <a class="ari-wordmark" href="index.html" aria-label="ARI Home">ARI</a>
    <p class="ari-engine-line"><span>RE</span> BIRTH ENGINE</p>
  </div>

  <button class="profile-btn" id="authButton" type="button">Sign In</button>
</header>

<nav id="sideMenu" class="side-menu ari-side-menu">
  ...
  <a href="log.html?mode=history">💭 Conversation History</a>

  <div class="nav-dashboard-card">
    <p class="nav-dashboard-label">CALORIES LEFT</p>

    <div class="nav-meter-wrap">
      <svg class="nav-meter" viewBox="0 0 220 130">
        <path class="nav-meter-bg" d="M 25 105 A 85 85 0 0 1 195 105" pathLength="100" />
        <path class="nav-meter-fill" id="navMeterFill" d="M 25 105 A 85 85 0 0 1 195 105" pathLength="100" />
      </svg>

      <div class="nav-meter-text">
        <h2 id="caloriesLeftText">2,100</h2>
        <span><span id="caloriesConsumedText">0</span> / <span id="dailyGoalText">2,100</span> kcal</span>
      </div>
    </div>

    <a href="log.html?mode=today" class="nav-dashboard-link">Open Daily Intake</a>
  </div>
</nav>

<div id="menuOverlay" class="menu-overlay" onclick="toggleMenu()"></div>

<section class="ari-welcome" id="ariWelcome">
  <img
    id="ariHero"
    class="ari-hero-img"
    data-ari-mascot
  ...
  <h1 class="ari-welcome-question">How can I help today?</h1>
</section>

<section class="ari-conversation-shell" id="ariConversationShell">
  <div id="ariMessages" class="ari-thread"></div>
...
<div class="ari-bottom-input">
  <div class="ari-input-shell">
    <textarea
      id="ariInput"
      rows="1"
...
<script src="calbuddy-core.js?v=3.5.8"></script>
<script src="ari/ari-rebirth-app-bridge.js?v=1.7.4"></script>
  ...
  ariBlinkInterval = setInterval(() => {
    ...
    if (ariFirstReplyCompleted) {
      ...
      ariBlinkTimeout = setTimeout(() => {
        if (!ariBusy) setAriAvatar("avatarOpen");
      }, 150);
    ...
    clearTimeout(ariBlinkTimeout);
    ariBlinkTimeout = setTimeout(() => {
      if (!ariBusy) setAriHero("heroOpen");
    }, 150);
  }, 30000);
}
...
function chooseAriReaction(message = "") {
  ...
  if (text.includes("fat") || text.includes("gained") || text.includes("weight gain")) {
    return { pose: "concerned" };
  }
...
function applyAriAfterResponseEmotion(message = "", reply = "") {
  ...
  if (combined.includes("great job") || combined.includes("congrats") || combined.includes("nice work")) {
    setAriPose("celebrate");
  } else if (combined.includes("sorry") || combined.includes("rough")) {
    setAriPose("concerned");
  } else if (combined.includes("yes") || combined.includes("good news")) {
    setAriPose("goodNews");
  } else {
    setAriPose("idleOpen");
  }
