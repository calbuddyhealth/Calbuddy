-- ARI Circle Find Friends Local V1
-- Privacy-safe people discovery around the viewer's chosen Circle search area.
-- Candidate coarse coordinates and private area labels never leave this function.
    ...
    select
      ...
      case
        ...
        when viewer_area is not null
          then (
            position(lower(viewer_area) in lower(coalesce(s.profile_location, ''))) > 0
            or position(lower(coalesce(s.profile_location, '')) in lower(viewer_area)) > 0
            or position(lower(viewer_area) in lower(coalesce(s.private_area, ''))) > 0
            or position(lower(coalesce(s.private_area, '')) in lower(viewer_area)) > 0
          )
        else false
      end as is_nearby
    from scored s
...
comment on function public.ari_circle_find_friends_v1(text, integer) is
  'Returns eligible friend-discovery profiles using the caller search radius and mutual accepted connections without exposing private search coordinates.';
