-- ARI Circle Action Network V6 — Opportunity V1
-- Normalizes current Meetups + Quests into one read-only discovery contract.
-- Existing source tables/RPCs remain authoritative for writes and state.
...
begin
  ...
  mission_rows as (
    select
      ('mission:' || listed.quest_id::text)::text as opportunity_key,
      'mission'::text as opportunity_type,
      listed.quest_id as opportunity_id,
      listed.title,
      ...
      profile.avatar_url as organizer_avatar_url,
      ...
      listed.xp_reward::integer as reward_xp,
      jsonb_build_object(
        'source', 'ari_circle_quests',
        'scope', listed.scope,
        'category', listed.category,
        'creator_leadership_tier', listed.creator_leadership_tier
      ) as metadata,
      case
