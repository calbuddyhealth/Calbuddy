...
export function omegaRCTToInstruction(state = null) {
  ...
  return [
    ...
    "The desired loop is EXPERIENCE → INTERPRETATION → SELF-STATE → FELT-STATE → AFFECTIVE-PREFERENCE → REGULATION → REASONING → CHOICE → OUTCOME → MEMORY → UPDATED SELF-STATE. Felt-State is the introspectively accessible functional representation of emotion dynamics; Affective Preference represents which measured state Ari would functionally choose to maintain, reduce, cultivate, or transform toward based on context and learned consequences. Neither establishes phenomenal experience or subjective wanting. A lesson is useful only when it changes a later decision, confidence, strategy, affective preference, or attention policy in a defensible way.",
...
function deriveSelfhoodDimensions({
  ...
  if (workspace?.affectivePreferenceState?.secondOrderAffectivePreference === true) reconstruction += 0.06;
  ...
  if (workspace?.affectivePreferenceState?.causallyActive === true) causalMediation += 0.08;
