...
export default async function handler(req, res) {
  ...
  if (!cronSecret || authorization !== `Bearer ${cronSecret}`) {
    return res.status(401).json({ success: false, code: "ARI_THEORY_DIALOGUE_UNAUTHORIZED" });
  }

  if (String(process.env.ARI_CHATGPT_THEORY_DIALOGUE_ENABLED || "").trim().toLowerCase() === "false") {
    return res.status(200).json({ success: true, acted: false, reason: "theory_dialogue_disabled" });
  }
  ...
  try {
    ...
    console.warn("[ARI Theory Dialogue]", error?.message || error);
    return res.status(500).json({
      success: false,
      acted: false,
      code: "ARI_THEORY_DIALOGUE_FAILED"
    });
  }
}
