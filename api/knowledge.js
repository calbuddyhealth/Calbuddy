...
// Version: 8.2.1-experimental
// Purpose: minimum-disclosure server-side OpenAI cognitive transport.
// =====================================================
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const requestText = firstText([
      body.requestText,
      packet.requestText,
      packet.request?.effective,
      packet.request?.resolved,
      packet.request?.original,
      packet.currentTurn?.effectiveText,
...
Interpret the user's current request yourself. Decide what matters, choose the response strategy, and write the authoritative user-facing answer.
Do not expose private chain-of-thought. Return conclusions and concise rationale only when useful.
Never claim to know stored profile, health, nutrition, account, memory, preference, application, or developer-file facts unless the current request states them.
...
ARI APPLICATION ACTIONS:
When the user explicitly asks Ari to log a meal, propose operation "log_meal". Do not propose any other application write unless the current request names the exact operation and the local application can independently validate it. Each proposed action must be an object with: operation, payload, authorization. Set authorization to "explicit_user_request" only when the current user request actually asks for that change. Do not invent record IDs, exercise IDs, connection IDs, or other identifiers required for execution.
...
Return JSON only with: interpretation, semanticFrame, reasoningDecision, responseStrategy, authoritativeDraft, proposedActions, evidenceReferences, warnings.`;
    ...
    // Deliberately disclose only the current request. The browser may carry
    // local continuity, memories, preferences, app state, or developer
    // evidence for deterministic handling, but this endpoint never forwards
    // those fields to the external model.
    ...
    if (!response.ok) {
      return res.status(response.status || 502).json({
        ...failure("openai_provider_failure", provider?.error?.message || raw || "OpenAI request failed."),
        model: MODEL,
    ...
    try {
      ...
      result = {
        interpretation: { summary: requestText, primaryIntent: "openai_interpreted_request" },
        semanticFrame: { primaryIntent: "openai_interpreted_request", topic: null, userGoal: requestText },
        reasoningDecision: { answerable: true, decision: "respond" },
        responseStrategy: { mode: "direct", tone: "adaptive", requirements: [] },
        authoritativeDraft: modelText,
        proposedActions: [],
        evidenceReferences: [],
        warnings: ["provider_output_was_not_json"]
      };
    }
    ...
    const cognitiveReasoningResult = {
      ...
      draftResponse: draft,
      proposedActions: Array.isArray(result.proposedActions) ? result.proposedActions : [],
      semanticFrame: object(result.semanticFrame, { primaryIntent: "openai_interpreted_request" }),
      responseStrategy: object(result.responseStrategy, { mode: "direct" }),
      model: MODEL,
      source: "openai",
      ready: true,
      modelInvocation: {
      ...
      authority: {
        ...
        draftSource: "openai",
        proposedActionSource: "openai",
        actionExecutionSource: "application_binding"
      }
    };
