...
const MODEL = process.env.OPENAI_REASONING_MODEL || process.env.OPENAI_MODEL || "gpt-5-mini";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const requestText = firstText([body.requestText, packet.request?.effective, packet.request?.resolved, packet.request?.original, packet.currentTurn?.effectiveText, packet.currentTurn?.originalText, body.message]);
    ...
    const context = {
      ...
      currentTurn: packet.currentTurn || null,
      continuity: packet.continuity || packet.continuityContext || null,
      memory: packet.memory || packet.memoryContext || null,
      evidence: packet.evidence || packet.evidenceContext || null,
      knowledge: packet.knowledge || packet.knowledgeContext || null,
      preferences: packet.preferenceContext || body.preferenceContext || null,
      restrictions: packet.restrictions || packet.safety || body.restrictions || null,
      applicationContext: packet.applicationContext || body.applicationContext || null,
      developerContext: packet.developerContext || body.developerContext || null,
      operationContract: body.operationContract || packet.operationContract || null,
      instructions: body.instructions || packet.instructions || null
    };
...
Return JSON only with: interpretation, semanticFrame, reasoningDecision, responseStrategy, authoritativeDraft, proposedActions, evidenceReferences, warnings.`;
    ...
    try {
      response = await fetch(OPENAI_URL, {
        ...
        body: JSON.stringify({
          model: MODEL,
          messages: [{ role: "developer", content: developerPrompt }, { role: "user", content: userPrompt }],
    ...
    try { result = JSON.parse(modelText); }
      ...
      result = {
        ...
        proposedActions: [], evidenceReferences: [], warnings: ["provider_output_was_not_json"]
      };
    }
    ...
    const cognitiveReasoningResult = {
      ...
      source: "openai",
      ready: true,
      modelInvocation: { available: true, attempted: true, succeeded: true, model: MODEL, source: "api/knowledge.js", usage: provider?.usage || null },
      authority: { semanticSource: "openai", reasoningSource: "openai", responseStrategySource: "openai", draftSource: "openai" }
    };
